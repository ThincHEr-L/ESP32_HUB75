...
void HUB75E_displayBufferPixels() {
	...
	for(int row = 0; row < VERTICAL_PIXELS;) {
		...
		switch (addressingMode)
		{
			case HUB75EAddressingModeABCDE:
				...
				HUB75E_setPin(PinADDRE, (row & 0x10) >> 4);
				break;
		...
		HUB75E_setPin(PinLATCH, 0);
		HUB75E_DelayUs(1); //Latch delay
		//ESP_LOGI("HUB75E", "Row: %d", row);
		row++; //Clock delay
		HUB75E_setPin(PinLATCH, 1);


		HUB75E_setPin(PinOE, 0); //Setting OE low for turning on all LEDs
